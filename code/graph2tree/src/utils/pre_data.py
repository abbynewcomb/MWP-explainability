...
class Lang:
    ...
    def trim(self, min_count):  # trim words below a certain count threshold
        ...
        print(
            ...
            % (
                len(keep_words),
                len(self.index2word),
                len(keep_words) / len(self.index2word),
            )
        )
    ...
    def build_input_lang(
        self, logger, trim_min_count
    ):  # build the input lang vocab and dict
        if trim_min_count > 0:
    ...
    def build_output_lang(
        self, generate_num, copy_nums
    ...
    def build_output_lang_for_tree(
        self, generate_num, copy_nums
    ):  # build the output lang vocab and dict
...
def load_mawps_data(filename):  # load the json data to list(dict()) for MAWPS
    ...
    for d in data:
        ...
        if (
            "lQueryVars" in d and len(d["lQueryVars"]) == 1
        ):  # When Equations are annotated with variables
            v = d["lQueryVars"][0]
        ...
        if (
            len(set(x) - set("0123456789.+-*/()=xX")) != 0
        ):  # If equation has anything not in the set on RHS of -
            continue
...
def prepare_data(
    ...
    if pairs_trained != None:
        ...
        for pair in pairs_trained:
            ...
            for word in pair[1]:  # For each token in equation
                ...
                flag_not = True
                if (
                    word not in output_lang.index2word
                ):  # If token is not in output vocab
                    flag_not = False
                    for i, j in enumerate(pair[2]):
                        if j == word:
                            temp_num.append(
                                i
                            )  # Append number list index of token not in output vocab

                if (
                    not flag_not and len(temp_num) != 0
                ):  # Equation has an unknown token and it is a number present in number list (could be default number with freq < 5)
                    num_stack.append(temp_num)
                if (
                    not flag_not and len(temp_num) == 0
                ):  # Equation has an unknown token but it is not a number from number list
    ...
    logger.debug(
        "Indexed {} words in input language, {} words in output".format(
...
def prepare_de_data(
    pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False
):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    ...
    for pair in pairs_trained:
        ...
        train_pairs.append(
            [input_cell, len(input_cell), pair[1], 0, pair[2], pair[3], num_stack]
        )
    print(
        "Indexed %d words in input language, %d words in output"
        % (input_lang.n_words, output_lang.n_words)
    )
...
# num net graph
def get_lower_num_graph(
    max_len, sentence_length, num_list, id_num_list, contain_zh_flag=True
):
...
def get_attribute_between_graph(
    input_batch,
    max_len,
    id_num_list,
    sentence_length,
...
def get_quantity_between_graph(
    max_len, id_num_list, sentence_length, quantity_cell_list, contain_zh_flag=True
):
    diag_ele = np.zeros(max_len)
    for i in range(sentence_length):
        diag_ele[i] = 1
...
def get_single_batch_graph(input_batch, input_length, group, num_value, num_pos):
    ...
    for i in range(len(input_length)):
        ...
        graph_newc = get_quantity_cell_graph(
            max_len, id_num_list, sentence_length, quantity_cell_list
        )
        graph_greater = get_greater_num_graph(
            max_len, sentence_length, num_list, id_num_list
        )
        graph_lower = get_lower_num_graph(
            max_len, sentence_length, num_list, id_num_list
        )
        graph_quanbet = get_quantity_between_graph(
            max_len, id_num_list, sentence_length, quantity_cell_list
        )
        graph_attbet = get_attribute_between_graph(
            input_batch_t, max_len, id_num_list, sentence_length, quantity_cell_list
        )
...
def get_single_example_graph(input_batch, input_length, group, num_value, num_pos):
    ...
    graph_attbet = get_attribute_between_graph(
        input_batch, max_len, id_num_list, sentence_length, quantity_cell_list
    )
    graph_greater = get_greater_num_graph(
        max_len, sentence_length, num_list, id_num_list
    )
...
def prepare_de_train_batch(
    pairs_to_batch, batch_size, output_lang, rate, english=False
):
    pairs = []
    ...
    while idx < len(ex):
        ...
        if (s == "*" or s == "+") and random.random() < rate:
            ...
            if s == "+":
                ...
                while not (
                    lidx == -1
                    or ((ex[lidx] == "+" or ex[lidx] == "-") and flag == 0)
                    or flag == 1
                ):
                ...
                while not (
                    ridx == len(ex)
                    or ((ex[ridx] == "+" or ex[ridx] == "-") and flag == 0)
                    or flag == -1
                ):
                ...
                if flag == -1:
                    ...
                    ridx -= 1
            if lidx > 0 and (
                (s == "+" and ex[lidx - 1] == "-") or (s == "*" and ex[lidx - 1] == "/")
            ):
                lidx -= 1
...
def allocation(ex_copy, rate):
    ...
    lex = len(ex)
    while idx < len(ex):
        if (ex[idx] == "/" or ex[idx] == "*") and (
            ex[idx - 1] == "]" or ex[idx - 1] == ")"
        ):
            ridx = idx + 1
            r_allo = []
            r_last = []
            ...
            if not flag_al:
                ...
                while temp_idx < idx - 1:
                    ...
                    if flag == 0:
                        if ex[temp_idx] == "+" or ex[temp_idx] == "-":
                            temp_res += (
                                ex[lidx:temp_idx] + [ex[idx]] + r_allo + [ex[temp_idx]]
                            )
                            lidx = temp_idx + 1
                    temp_idx += 1
            ...
            if not flag_al:
                ...
                while temp_idx < idx - 1:
                    ...
                    if flag == 1:
                        if ex[temp_idx] == "+" or ex[temp_idx] == "-":
                            all_res += (
                                l_allo + [ex[idx]] + ex[lidx:temp_idx] + [ex[temp_idx]]
                            )
                            lidx = temp_idx + 1
                    if flag == 0:
                        break
