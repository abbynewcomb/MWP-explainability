...
class EncoderRNN(nn.Module):
    def __init__(
        self, input_size, embedding_size, hidden_size, n_layers=2, dropout=0.5
    ):
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs, seq_mask):
        ...
        output = self.out(
            torch.tanh(
                self.concat(torch.cat((rnn_output.squeeze(0), context.squeeze(1)), 1))
            )
        )
...
class EncoderSeq(nn.Module):
    ...
    def forward(self, embedded, input_lengths, orig_idx, batch_graph, hidden=None):
        ...
        problem_output = (
            pade_outputs[-1, :, : self.hidden_size]
            + pade_outputs[0, :, self.hidden_size :]
        )
        pade_outputs = (
            pade_outputs[:, :, : self.hidden_size]
            + pade_outputs[:, :, self.hidden_size :]
        )  # S x B x H
        # pdb.set_trace()
...
class Prediction(nn.Module):
    ...
    def forward(
        ...
        current_attn = self.attn(
            current_embeddings.transpose(0, 1), encoder_outputs, seq_mask
        )
