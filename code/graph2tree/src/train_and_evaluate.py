...
def generate_rule_mask(
    ...
    rule_mask = torch.FloatTensor(batch_size, nums_start + copy_nums).fill_(
        -float("1e12")
    )
    if english:
        if decoder_input[0] == word2index["SOS"]:
            for i in range(batch_size):
                res = (
                    [_ for _ in range(nums_start, nums_start + nums_batch[i])]
                    + [word2index["("]]
                    + generate_nums
                )
                for j in res:
        ...
        for i in range(batch_size):
            ...
            if decoder_input[i] >= nums_start or decoder_input[i] in generate_nums:
                ...
                decoder_input[i] == word2index["["]
                or decoder_input[i] == word2index["("]
            ):
                res += (
                    [_ for _ in range(nums_start, nums_start + nums_batch[i])]
                    + [word2index["("]]
...
def generate_pre_tree_seq_rule_mask(
    decoder_input,
    nums_batch,
    word2index,
    batch_size,
...
def generate_post_tree_seq_rule_mask(
    decoder_input,
    nums_batch,
    word2index,
    batch_size,
...
def mask_num(
    encoder_outputs, decoder_input, embedding_size, nums_start, copy_nums, num_pos
):
    ...
    num_mask_encoder = num_mask_encoder.repeat(
        *repeat_dims
    )  # B x 1 -> B x Decoder_embedding_size

    all_embedding = encoder_outputs.transpose(0, 1).contiguous()
    all_embedding = all_embedding.view(
        -1, encoder_outputs.size(2)
    )  # S x B x H -> (B x S) x H
...
def compute_postfix_tree_result(test_res, test_tar, output_lang, num_list, num_stack):
    ...
    try:
        if (
            abs(compute_postfix_expression(test) - compute_postfix_expression(tar))
            < 1e-4
        ):
            return True, False, test, tar
        else:
...
def get_all_number_encoder_outputs(
    encoder_outputs, num_pos, batch_size, num_size, hidden_size
):
    indices = list()
    sen_len = encoder_outputs.size(0)
    masked_index = []
    ...
    if random.random() < use_teacher_forcing:
        ...
        for t in range(max_target_length):
            ...
            all_hidden = torch.zeros(
                decoder_hidden.size(0), batch_size * beam_len, decoder_hidden.size(2)
            )
            ...
            for b_idx in range(len(beam_list)):
                ...
                score += beam_score
                beam_scores[
                    :, b_idx * decoder.output_size : (b_idx + 1) * decoder.output_size
                ] = score
                all_hidden[
                ...
                all_outputs[
                    :, batch_size * b_idx : batch_size * (b_idx + 1), :
...
def evaluate_attn(
    ...
    for di in range(max_length):
        ...
        for b_idx in range(len(beam_list)):
            ...
            beam_scores[
                current_idx
                * decoder.output_size : (current_idx + 1)
                * decoder.output_size
            ] = score
...
def train_tree(
    ...
    padding_hidden = torch.FloatTensor(
        [0.0 for _ in range(predict.hidden_size)]
    ).unsqueeze(0)
    batch_size = len(input_length)
    ...
    if config.embedding == "bert" or config.embedding == "roberta":
        ...
        for bat in range(len(group_batch)):
            try:
                new_group_batch.append(
                    [
                        index_retrieve[bat][index1]
                        for index1 in group_batch[bat]
                        if index1 < len(index_retrieve[bat])
                    ]
                )
            except:
                pdb.set_trace()

        batch_graph = get_single_batch_graph(
        ...
        embedded, input_length, orig_idx = sort_by_len(
            input_seq1, input_len1, gpu_init_pytorch(config.gpu)
        )
    else:
    ...
    encoder_outputs, problem_output = encoder(
        embedded, input_length, orig_idx, batch_graph
    )
    ...
    for t in range(max_target_length):
        ...
        left_childs = []
        for idx, l, r, node_stack, i, o in zip(
            range(batch_size),
            left_child.split(1),
            right_child.split(1),
            node_stacks,
...
def evaluate_tree(
    ...
    orig_idx = None
    if config.embedding == "bert" or config.embedding == "roberta":
        contextual_input = index_batch_to_words(
            [input_batch], [input_length], input_lang
        )
        input_seq1, input_len1, token_ids, index_retrieve = embedding(contextual_input)

        try:
            new_group_example = [
                index_retrieve[0][index1]
                for index1 in group_example
                if index1 < len(index_retrieve[0])
            ]
        except:
            pdb.set_trace()
...
def topdown_train_tree(
    ...
    for t in range(max_target_length):
        ...
        for idx, l, r, node_stack, i in zip(
            range(batch_size),
            left_child.split(1),
            right_child.split(1),
            node_stacks,
