...
def main():
    ...
    if config.full_cv:
        ...
        config.val_result_path = os.path.join(
            result_folder, "CV_results_{}.json".format(data_name)
        )
        fold_acc_score = 0.0
        folds_scores = []
        best_acc = []
        for z in range(5):
            ...
            if config.results:
                config.result_path = os.path.join(
                    result_folder, "val_results_{}.json".format(config.dataset)
                )
            ...
            train_ls, dev_ls = load_raw_data(
                data_path, config.dataset, mode=config.mode
            )
            pairs_trained, pairs_tested, generate_nums, copy_nums = transfer_num(
                train_ls, dev_ls, config.challenge_disp
            )
            ...
            predict_optimizer = torch.optim.Adam(
                predict.parameters(), lr=config.lr, weight_decay=config.weight_decay
            )
            generate_optimizer = torch.optim.Adam(
                generate.parameters(), lr=config.lr, weight_decay=config.weight_decay
            )
            merge_optimizer = torch.optim.Adam(
                merge.parameters(), lr=config.lr, weight_decay=config.weight_decay
            )

            logger.debug("Optimizers Initialized")
            logger.info("Initializing Schedulers...")

            embedding_scheduler = torch.optim.lr_scheduler.StepLR(
                embedding_optimizer, step_size=20, gamma=0.5
            )
            ...
            if config.mode == "train":
                ...
                for epoch in range(config.epochs):
                    ...
                    (
                        ...
                        num_pos_batches,
                        num_size_batches,
                    ) = prepare_train_batch(train_pairs, config.batch_size)
                    ...
                    for idx in range(len(input_lengths)):
                        ...
                        loss_total += loss
                        print(
                            "Completed {} / {}...".format(idx, len(input_lengths)),
                            end="\r",
                            flush=True,
                        )
                    ...
                    logger.debug(
                        "Training for epoch {} completed...\nTime Taken: {}".format(
                            epoch, time_since(time.time() - start)
                        )
                    )
                    ...
                    for test_batch in test_pairs:
                        ...
                        with open(config.outputs_path + "/outputs.txt", "a") as f_out:
                            ...
                            f_out.write(
                                "Source: "
                                + stack_to_string(
                                    sentence_from_indexes(input_lang, test_batch[0])
                                )
                                + "\n"
                            )
                            f_out.write(
                                "Target: "
                                + stack_to_string(
                                    sentence_from_indexes(output_lang, test_batch[2])
                                )
                                + "\n"
                            )
                            f_out.write(
                                "Generated: "
                                + stack_to_string(
                                    sentence_from_indexes(output_lang, test_res)
                                )
                                + "\n"
                            )
                            if config.nums_disp:
                                ...
                                tgt_nums = 0
                                pred_nums = 0
                                for k_tgt in sentence_from_indexes(
                                    output_lang, test_batch[2]
                                ):
                                    if k_tgt not in ["+", "-", "*", "/"]:
                                        tgt_nums += 1
                                for k_pred in sentence_from_indexes(
                                    output_lang, test_res
                                ):
                                    if k_pred not in ["+", "-", "*", "/"]:
                                        pred_nums += 1
                                f_out.write(
                                    "Numbers in question: " + str(src_nums) + "\n"
                                )
                                f_out.write(
                                    "Numbers in Target Equation: "
                                    + str(tgt_nums)
                                    + "\n"
                                )
                                f_out.write(
                                    "Numbers in Predicted Equation: "
                                    + str(pred_nums)
                                    + "\n"
                                )
                    ...
                    if float(value_ac) / eval_total > max_val_acc:
                        ...
                        if config.save_model:
                            save_checkpoint(
                                state, epoch, logger, config.model_path, config.ckpt
                            )
        ...
        train_ls, dev_ls = load_raw_data(
            data_path, config.dataset, is_train, mode=config.mode
        )
        pairs_trained, pairs_tested, generate_nums, copy_nums = transfer_num(
            train_ls, dev_ls, config.challenge_disp
        )
        ...
        if is_train:
            ...
            logger.info(
                "Vocab Files loaded from {}\nNumber of Words: {}".format(
                    vocab1_path, input_lang.n_words
                )
            )
        ...
        if is_train:
            ...
            predict = Prediction(
                hidden_size=config.hidden_size,
                op_nums=output_lang.n_words
                - config.copy_nums
                - 1
                - config.len_generate_nums,
                input_size=config.len_generate_nums,
                dropout=config.dropout,
            )
            generate = GenerateNode(
                hidden_size=config.hidden_size,
            ...
            for test_batch in test_pairs:
                ...
                test_res_ques.append(
                    stack_to_string(sentence_from_indexes(input_lang, test_batch[0]))
                )
                test_res_act.append(
                    stack_to_string(sentence_from_indexes(output_lang, test_batch[2]))
                )
                test_res_gen.append(
                    stack_to_string(sentence_from_indexes(output_lang, test_res))
                )
            ...
            logger.info(
                "Accuracy: {}".format(sum(test_res_scores) / len(test_res_scores))
            )
