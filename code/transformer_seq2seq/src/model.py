...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        self.scale = nn.Parameter(
            torch.ones(1)
        )  # nn.Parameter causes the tensor to appear in the model.parameters()
        ...
        )  # torch.arange(0, d_model, 2) gives 2i
...
class TransformerModel(nn.Module):
    def __init__(
        self, config, voc1, voc2, device, logger, EOS_tag="</s>", SOS_tag="<s>"
    ):
        ...
        self.pos_embedding1 = PositionalEncoding(
            self.config.d_model, self.config.dropout
        )
        ...
        self.pos_embedding2 = PositionalEncoding(
            self.config.d_model, self.config.dropout
        )
    ...
    def _initialize_optimizer(self):
        self.params = (
            ...
            + list(self.embedding2.parameters())
            + list(self.pos_embedding1.parameters())
            + list(self.pos_embedding2.parameters())
        )
        self.non_emb_params = (
            list(self.transformer.parameters())
            + list(self.fc_out.parameters())
    ...
    def trainer(self, ques, input_seq1, input_seq2, config, device=None, logger=None):
        ...
        self.loss = self.criterion(
            output.view(-1, output_dim), input_seq2[1:, :].reshape(-1),
        )
        ...
        with torch.no_grad():
            ...
            if self.config.embedding == "bert" or self.config.embedding == "roberta":
                ...
                memory = self.transformer.encoder(
                    self.pos_embedding1(self.embedding1(input_seq1))
                )
            ...
            for step in range(target_len):
                decoder_input = torch.LongTensor(input_list).to(
                    self.device
                )  # seq_len x bs

                decoder_output = self.fc_out(
                    self.transformer.decoder(
                        self.pos_embedding2(self.embedding2(decoder_input)), memory
                    )
                )  # seq_len x bs x voc2.nwords
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for data in train_dataloader:
            ...
            if config.show_train_acc:
                ...
                _, decoder_output = model.greedy_decode(
                    ques, sent1_var, sent2_var, input_len2, validation=True
                )
...
def run_validation(
    ...
    for data in val_dataloader:
        ...
        val_loss, decoder_output = model.greedy_decode(
            ques, sent1_var, sent2_var, input_len2, validation=True
        )
