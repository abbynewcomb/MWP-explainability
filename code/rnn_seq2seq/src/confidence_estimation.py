...
def similarity_based_conf(test_ques, train_ques, model, sim_criteria="bert_score"):
    ...
    if sim_criteria == "bert_score":
        similarities = bert_sim(
            test_ques, train_ques, model
        )  # [Batch Size x |Training Data|]
...
def bert_sim(queries, keys, model):
    ...
    with torch.no_grad():
        ...
        keys_rep = torch.cat(
            [
                model.bert(keys[i : min(i + 16, len(keys)),])[0][:, 0]
    ...
    sims = np.dot(
        queries_rep / np.linalg.norm(queries_rep, axis=-1, keepdims=True),
