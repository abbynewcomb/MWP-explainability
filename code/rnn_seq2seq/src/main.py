...
def load_data(config, logger):
    ...
    if config.mode == "train":
        ...
        val_set = TextDataset(
            ...
            max_length=config.max_length,
            is_debug=config.debug,
            grade_info=config.grade_disp,
            type_info=config.type_disp,
    ...
    if config.full_cv:
        ...
        for z in range(5):
            ...
            if is_train:
                ...
                logger.info(
                    "Vocab loaded with number of words : {}".format(voc1.nwords)
                )
            else:
            ...
            if is_train:
                ...
                if checkpoint == None:
                    ...
                    (
                        ...
                        config, model, config.mode, checkpoint, logger, device
                    )
        ...
        if is_train:
            ...
            (
                ...
                voc1,
                voc2,
            ) = load_checkpoint(config, model, config.mode, checkpoint, logger, device)
