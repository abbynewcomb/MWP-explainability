...
class Seq2SeqModel(nn.Module):
    def __init__(
        ...
        nn.init.uniform_(
            self.embedding2.weight, -1 * self.config.init_range, self.config.init_range
        )

        if self.config.embedding == "bert":
            self.embedding1 = BertEncoder(
                self.config.emb_name, self.device, self.config.freeze_emb
            )
        elif self.config.embedding == "roberta":
            self.embedding1 = RobertaEncoder(
                self.config.emb_name, self.device, self.config.freeze_emb
            )
        elif self.config.embedding == "word2vec":
            self.config.emb1_size = 300
            self.embedding1 = nn.Embedding.from_pretrained(
                torch.FloatTensor(self._form_embeddings(self.config.word2vec_bin)),
                freeze=self.config.freeze_emb,
            )
        else:
            self.embedding1 = nn.Embedding(self.voc1.nwords, self.config.emb1_size)
            nn.init.uniform_(
                self.embedding1.weight,
                -1 * self.config.init_range,
    ...
    def _form_embeddings(self, file_path):
        weights_all = models.KeyedVectors.load_word2vec_format(
            file_path, limit=200000, binary=True
        )
    ...
    def _initialize_optimizer(self):
        self.params = (
            list(self.embedding1.parameters())
            + list(self.encoder.parameters())
            + list(self.decoder.parameters())
        )

        if self.config.separate_opt:
            self.emb_optimizer = AdamW(
                self.embedding1.parameters(), lr=self.config.emb_lr, correct_bias=True
            )
            ...
            if self.config.opt == "adam":
                self.optimizer = optim.Adam(
                    [
                        {
                            "params": self.embedding1.parameters(),
    ...
    def trainer(
        ...
        if self.config.embedding == "bert" or self.config.embedding == "roberta":
            ...
            # input_len1: List [BS]
            sorted_seqs, sorted_len, orig_idx = sort_by_len(
                input_seq1, input_len1, self.device
            )
        ...
        encoder_outputs, encoder_hidden = self.encoder(
            sorted_seqs, sorted_len, orig_idx, self.device
        )

        self.loss = 0

        decoder_input = torch.tensor(
            [self.SOS_token for _ in range(input_seq1.size(1))], device=self.device
        )

        if config.cell_type == "lstm":
            decoder_hidden = (
                encoder_hidden[0][: self.decoder.nlayers],
                encoder_hidden[1][: self.decoder.nlayers],
            )
        else:
            decoder_hidden = encoder_hidden[: self.decoder.nlayers]

        use_teacher_forcing = (
            True if random.random() < self.config.teacher_forcing_ratio else False
        )
    ...
    def greedy_decode(
        ...
        with torch.no_grad():
            ...
            loss = 0.0
            decoder_input = torch.tensor(
                [self.SOS_token for i in range(input_seq1.size(1))], device=self.device
            )
    ...
    def obtain_hidden(
        ...
        with torch.no_grad():
            ...
            decoded_words = [[] for _ in range(input_seq1.size(1))]
            decoder_attentions = []
...
def train_model(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for data in train_dataloader:
            ...
            if config.show_train_acc:
                ...
                _, decoder_output, _ = model.greedy_decode(
                    ques, sent1_var, sent2_var, input_len1, input_len2, validation=True
                )
                temp_acc_cnt, temp_acc_tot, _ = cal_score(
                    decoder_output, nums, ans, data["eqn"]
                )
        ...
        val_bleu_epoch, val_loss_epoch, val_acc_epoch = run_validation(
            config=config,
            model=model,
        ...
        if val_acc_epoch > max_val_acc:
            ...
            if config.save_model:
                save_checkpoint(
                    state, epoch + epoch_offset, logger, config.model_path, config.ckpt
                )
            estop_count = 0
        else:
            estop_count += 1
        ...
        if estop_count > config.early_stopping:
            logger.debug(
                "Early Stopping at Epoch: {} after no improvement in {} epochs".format(
                    epoch, estop_count
                )
            )
            break
    ...
    if config.save_writer:
        writer.export_scalars_to_json(
            os.path.join(config.board_path, "all_scalars.json")
        )
        writer.close()
        ...
        with open(config.outputs_path + "/outputs.txt", "a") as f_out:
            ...
            for i in range(len(sent1s[:display_n])):
                try:
                    ...
                    f_out.write(
                        "Generated: " + stack_to_string(decoder_output[i]) + "\n"
                    )
                    if config.challenge_disp:
                        f_out.write("Type: " + str(type1[i]) + "\n")
                    ...
                    if config.nums_disp:
                        ...
                        f_out.write(
                            "Numbers in Target Equation: " + str(tgt_nums) + "\n"
                        )
                        f_out.write(
                            "Numbers in Predicted Equation: " + str(pred_nums) + "\n"
                        )
...
def estimate_confidence(config, model, dataloader, logger):
    ...
    total_batches = len(dataloader)
    logger.info(
        "Beginning estimating confidence based on {} criteria".format(config.conf)
    )
    start = time()
    for data in dataloader:
        ...
        if config.conf == "posterior":
            ...
            decoded_words, confidence = similarity_based_conf(
                ques, train_ques, model, sim_criteria=config.sim_criteria
            )
        else:
            # TODO: Implement other methods
            raise ValueError(
                "Other confidence methods not implemented yet. Use -conf posterior"
            )

        if not config.adv:
            correct_or_not = [
                cal_score([decoded_words[i]], [nums[i]], [ans[i]])[0]
    ...
    results_df = pd.DataFrame(
        [questions, act_eqns, gen_eqns, scores, confs]
    ).transpose()
    results_df.columns = [
        "Question",
        "Actual Equation",
    ...
    if config.conf != "similarity":
        csv_file_path = os.path.join(
            "ConfidenceEstimates",
            config.dataset + "_" + config.run_name + "_" + config.conf + ".csv",
        )
    else:
        csv_file_path = os.path.join(
            "ConfidenceEstimates",
...
def get_hiddens(config, model, val_dataloader, voc1, voc2, device):
    ...
    for data in val_dataloader:
        if len(data["ques"]) == config.batch_size:
            ...
            hidden, decoder_output = model.obtain_hidden(
                config, ques, sent1_var, sent2_var, input_len1, input_len2
            )
