...
class TextDataset(Dataset):
    ...
    def __init__(
        ...
        if grade_info and type_info:
            all_sents = zip(
                self.ques, self.eqn, self.nums, self.ans, self.grade, self.type
            )
        ...
        if grade_info and type_info:
            self.ques, self.eqn, self.nums, self.ans, self.grade, self.type = zip(
                *all_sents
            )
        elif grade_info and not type_info:
    ...
    def __getitem__(self, idx):
        ...
        if self.grade_info and self.type_info:
            ...
            type1 = self.type[idx]
            return {
                "ques": self.curb_to_length(ques),
                "eqn": self.curb_to_length(eqn),
                "nums": nums,
                "ans": ans,
                "grade": grade,
                "type": type1,
