        ...
        if self.config.depth > 1:
            for z in range(self.config.depth - 1):
                encoder_hidden = torch.cat(
                    (encoder_hidden, encoder_hidden_single), dim=0
                )
...
def train_model(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        val_bleu_epoch, val_loss_epoch, val_acc_epoch, val_attn_wts = run_validation(
            config=config,
            model=model,
    ...
    for data in dataloader:
        ...
        for yz in range(len(decoder_output)):
            if disp_corr[yz] == 1 and len(decoder_output[yz]) == 3:
                attn_wts_ls.append(
                    [data["ques"][yz], data["eqn"][yz], decoder_attn[0][yz]]
                )
        ...
        if config.mode == "test":
            ...
            scores += [
                cal_score([decoder_output[i]], [nums[i]], [ans[i]])[0]
